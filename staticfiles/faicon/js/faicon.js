...
$( document ).ready(function() {
    ...
    $( "#faicon-modal-cont" ).load( "/faicon/render_icon_list_modal/" ,function() {
        ...
        $('body').on('click', '.faicon-add', function(){
            $('body').addClass('faicon-active');
            $('.faicon-screen').addClass('show');
            icon_field = $(this).siblings('input').first();
            // if #faicon-style-select is using select2 destroy
            if ($('#faicon-style-select').hasClass('select2-hidden-accessible')) {
                $('#faicon-style-select').select2('destroy');
            }
        })

        $('body').on('click', '.faicon-delete', function(){
